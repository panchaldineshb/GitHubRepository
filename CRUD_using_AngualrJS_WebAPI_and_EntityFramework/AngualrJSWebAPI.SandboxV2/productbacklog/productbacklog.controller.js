...
.controller('ProductBacklogController', ['$scope', '$rootScope', '$location', 'ProductBacklog',
    function ($scope, $rootScope, $location, ProductBacklog) {
        $scope.userData = [];
        ...
        $scope.all = function () {
            ...
            ProductBacklog.GetAll(function (response) {
        ...
        $scope.cancel = function () {
            // reset login status
            $scope.dataLoading = true;
            ProductBacklog.ClearRegistration($scope.Name, $scope.Description, $scope.City, $scope.Address, $scope.ContactNo, $scope.EmailId, function (response) {
                if (response.success) {
                    $location.path('/');
                } else {
                    $scope.error = response.message;
        ...
        $scope.register = function () {
            $scope.dataLoading = true;
            ProductBacklog.Register($scope.Name, $scope.Description, $scope.City, $scope.Address, $scope.ContactNo, $scope.EmailId, function (response) {
                if (response.success) {
                    $location.path('/');
                } else {
                    $scope.error = response.message;
