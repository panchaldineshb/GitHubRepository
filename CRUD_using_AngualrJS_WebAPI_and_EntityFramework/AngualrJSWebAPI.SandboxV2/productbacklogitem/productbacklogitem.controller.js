...
.controller('ProductBacklogItemController', ['$scope', '$rootScope', '$location', 'ProductBacklogItem',
    function ($scope, $rootScope, $location, ProductBacklogItem) {
        ...
        $scope.all = function () {
            ...
            ProductBacklogItem.GetAll(function (response) {
        ...
        $scope.cancel = function () {
            // reset login status
            $scope.dataLoading = true;
            ProductBacklogItem.ClearRegistration($scope.Name, $scope.Description, $scope.City, $scope.Address, $scope.ContactNo, $scope.EmailId, function (response) {
                if (response.success) {
                    $location.path('/');
                } else {
                    $scope.error = response.message;
        ...
        $scope.register = function () {
            $scope.dataLoading = true;
            ProductBacklogItem.Register($scope.Name, $scope.Description, $scope.City, $scope.Address, $scope.ContactNo, $scope.EmailId, function (response) {
                if (response.success) {
                    $location.path('/');
                } else {
                    $scope.error = response.message;
