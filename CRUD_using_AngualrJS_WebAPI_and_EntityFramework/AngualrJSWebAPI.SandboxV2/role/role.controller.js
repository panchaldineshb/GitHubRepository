...
.controller('RoleController', ['$scope', '$rootScope', '$location', 'Role',
    function ($scope, $rootScope, $location, Role) {
        ...
        $scope.cancel = function () {
            // reset login status
            $scope.dataLoading = true;
            Role.ClearRegistration($scope.Name, $scope.Description, $scope.City, $scope.Address, $scope.ContactNo, $scope.EmailId, function (response) {
                if (response.success) {
                    $location.path('/');
                } else {
                    $scope.error = response.message;
        ...
        $scope.register = function () {
            $scope.dataLoading = true;
            Role.Register($scope.Name, $scope.Description, $scope.City, $scope.Address, $scope.ContactNo, $scope.EmailId, function (response) {
                if (response.success) {
                    $location.path('/');
                } else {
                    $scope.error = response.message;
