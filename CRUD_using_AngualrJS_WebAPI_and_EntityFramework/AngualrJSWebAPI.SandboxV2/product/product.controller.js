...
angular
.module('phonecatApp')
.controller('ProductController', ['$scope', '$rootScope', '$location', 'Product',
    function ($scope, $rootScope, $location, Product) {
        $scope.userData = [];
        ...
        $scope.cancel = function () {
            // reset login status
            $scope.dataLoading = true;
            Product.ClearRegistration($scope.Name, $scope.Description, $scope.City, $scope.Address, $scope.ContactNo, $scope.EmailId, function (response) {
                if (response.success) {
                    $location.path('/');
                } else {
                    $scope.error = response.message;
        ...
        $scope.register = function () {
            $scope.dataLoading = true;
            Product.Register($scope.Name, $scope.Description, $scope.City, $scope.Address, $scope.ContactNo, $scope.EmailId, function (response) {
                if (response.success) {
                    $location.path('/');
                } else {
                    $scope.error = response.message;
